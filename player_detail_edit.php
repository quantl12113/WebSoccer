<?php
    ...
    session_start();
    if($_SESSION == NULL) {
        header('Location: player_detail.php?&id='.$_GET['id']);
    }
    if($_SESSION != NULL) {
    ...
    <div class="container">
      ...
      <div class="row header"><!-- Begin Header -->
        ...
        <div class="span12 gallery-single">
            ...
            AND cauthu.\"MSCAUTHU\" = '$_GET[id]'";
            $sth=$db->prepare($sql_player);
            $sth->execute();
            $cauthu=$sth->fetch(PDO::FETCH_ASSOC);
            if(0<$cauthu['day'] && $cauthu['day']<10){
                    $cauthu['day']="0".$cauthu['day'];
            }
            if(0<$cauthu['month'] && $cauthu['month']<10){
                    $cauthu['month']="0".$cauthu['month'];
            }
        ?>
            ...
            <div class="row">
                ...
                <div class="span5">
                    <img src="img/player/<?php echo $cauthu['HINHANH'] ?>"  alt="image" width="350px" height="350px">
                </div>
                <div class="span7">
                    <h2><input name="tencauthu" type="text" value="<?php echo $cauthu['TENCAUTHU'] ?>"></h2>
                    <ul class="project-info">
                        <li><h6>Nationality:</h6> <img src="img/nationality/<?php echo $cauthu['FLAG'] ?>" width="30px" height="20px">&ensp; 
                        ...
                        <li><h6>Club:</h6> 
                            <img src="img/bigLogo/<?php echo $cauthu['LOGO'] ?>" width="25px" height="25px" > &ensp;
                            <select name="msclb">
                                <?php
                                    foreach($list_clb as $value){
                                        if($value == $cauthu['MSCLB']) {
                        ...
                        <li><h6>Number:</h6>
                        	<input name="soao" type="number" value="<?php echo $cauthu['SOAO'] ?>">
                        </li>
                        <li><h6>Age:</h6> 
                        	<?php echo $cauthu['tuoi'] ?>
                        </li>
                        <li><h6>Date of birth:</h6> 
                        	<input name="ngaysinh" type="date" value="<?php echo $cauthu['year']."-".$cauthu['month']."-".$cauthu['day'] ?>">
                        </li>
                        <li><h6>Height:</h6> 
            	...
            	<button id="delete" class="btn btn-inverse pull-left" mscauthu="<?php echo $id?>">Delete</button>
            </div>
...
<?php
	if(isset($_POST['submit'])) {
		$data = $_POST;
		$sql_update = "UPDATE cauthu 
			SET \"HINHANH\" = '$cauthu[HINHANH]', \"TENCAUTHU\" = '$data[tencauthu]', \"QUOCTICH\" = '$data[quoctich]', \"MSCLB\" = '$data[msclb]', \"VITRI\" = '$data[vitri]', \"SOAO\" = '$data[soao]', \"NGAYSINH\" = '$data[ngaysinh]', \"CHIEUCAO\" = '$data[chieucao]' 
			WHERE \"MSCAUTHU\" = '$id'";
...
<script>
	$('#delete').click(function() {
		location.href = "player_delete.php?&id=" + $(this).attr('mscauthu'); 
	});
